...
//		var curCapacityA, curCapacityB int //剩余水量
//		c := len(plants)
//		res := 0
...
//				}
//				if curCapacityB < plants[c-i-1] {
//					res++
//					curCapacityB = capacityB - plants[c-i-1]
//				} else {
//					curCapacityB -= plants[c-i-1]
//				}
//			}
...
//	}
//
// 思路还是维护两个水罐的容量，双指针移动，如果容量不够，则重新灌满水罐，如果容量足够，则减去当前水量
func minimumRefill(plants []int, capacityA int, capacityB int) int {
	curCapacityA, curCapacityB, left, right, res := capacityA, capacityB, 0, len(plants)-1, 0
	for left < right {
		...
		left++
		right--
		if right == left && max(curCapacityA, curCapacityB) < plants[left] {
			res++
		}
	}
	return res
